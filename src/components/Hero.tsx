...
function Progress({ openAt, closeAt }: { openAt: number; closeAt: number }) {
  ...
  const elapsed = Math.max(0, Math.min(total, Math.floor(now / 1000) - openAt));
  const pct = Math.round((elapsed / total) * 100);
  const remain = Math.max(0, closeAt * 1000 - now);
...
export default function Hero() {
  ...
  const [openAt, setOpenAt] = React.useState(0);
  ...
  const remainMs = Math.max(0, closeAt * 1000 - Date.now());
  ...
  return (
    <>
      ...
      <div>
        ...
        <div style={{ height: 10 }} />
        <Progress openAt={openAt} closeAt={closeAt} />
        <div style={{ height: 14 }} />
      ...
      >
        <img
          ...
          alt="BlueCat"
          style={{
            width: 300, height: 300, display: 'block', margin: '0 auto',
